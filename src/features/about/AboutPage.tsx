...
const values = [
  {
    label: "Innovation",
    icon: (
      <path d="M9 18h6M10 22h4M12 2a6 6 0 00-4 10.5c.6.55 1 1.32 1 2.5h6c0-1.18.4-1.95 1-2.5A6 6 0 0012 2z" />
    ),
  },
  {
    label: "Trust & Transparency",
    icon: (
      <path d="M11 21l-6.5-3.7a2 2 0 01-1-1.74V6.4a2 2 0 011-1.73L11 1l6.5 3.67a2 2 0 011 1.73v9.16a2 2 0 01-1 1.74L11 21z M8 11l2.2 2.2L15 8.5" />
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (
      <>
        ...
        <path d="M2.5 20c0-3.3 2.5-5.8 5.5-5.8S13.5 16.7 13.5 20" />
        <path d="M13.8 14.6c2.3.3 4.2 2.3 4.2 5.4" />
      </>
    ),
  },
];
...
/**
 * Contact/social buttons under a member's name.
 *
 * Only links the admin marked visible are rendered; members saved before the
 * links editor existed fall back to their single LinkedIn URL.
 */
function MemberLinks({
  ...
  const links = visibleMemberLinks(
    member.links?.length
      ? member.links
      : member.linkedin
        ? [
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-1 ${align === "center" ? "justify-center" : "justify-start"} ${className}`}
    >
      {links.map((link) => {
        const label = memberLinkLabel(link);
        return (
          <a
            key={link.id}
            ...
            className="inline-flex h-8 w-8 items-center justify-center text-primary transition-colors hover:text-accent"
          >
...
function MemberPhoto({
  ...
  return member.photo ? (
    <img
      src={member.photo}
      alt={member.name}
      className={`h-full w-full object-cover ${hover} ${className}`}
    />
  ) : (
    <div
      className={`h-full w-full flex items-center justify-center bg-linear-to-br from-[#0EA5E9]/15 to-[#072069]/15 text-[#072069] text-4xl font-extrabold ${hover} ${className}`}
    >
...
/**
 * Hover styling for the photo frame.
 *
 * It sits on the image container only — the name, role and link icons below
 * stay put. `group` here is what lets MemberPhoto zoom when the frame is
 * hovered. Note the transitioned property is `translate`, not `transform`:
 * Tailwind v4 compiles -translate-y-* to the standalone translate property.
 */
const PHOTO_HOVER =
  "group transition-[translate,box-shadow] duration-500 ease-out hover:shadow-xl motion-safe:hover:-translate-y-1";
...
function FounderCard({ member }: { member: Member }) {
  return (
    <div className="reveal w-full max-w-125 mx-auto">
      ...
      <h3 className="mt-5 text-xl font-extrabold text-[#0F1729]">
        {member.name}
      </h3>
      <p className="text-sm text-muted-foreground mt-1">{member.role}</p>
      <MemberLinks member={member} align="start" className="mt-3" />
    </div>
  );
}
...
function TeamCard({ member }: { member: Member }) {
  return (
    <div className="reveal text-center">
      ...
      <MemberLinks member={member} className="mt-2.5" />
    </div>
  );
}
...
function InternCard({ member }: { member: Member }) {
  return (
    <div className="reveal h-full">
      <div className="h-full bg-white rounded-2xl border border-border p-4 text-center shadow-sm">
        <div
          className={`aspect-square w-32 mx-auto rounded-xl overflow-hidden bg-secondary ${PHOTO_HOVER}`}
        >
          <MemberPhoto member={member} />
        </div>
        <h3 className="mt-4 text-base font-bold text-[#072069]">
          {member.name}
        </h3>
        ...
        <MemberLinks member={member} className="mt-3" />
      </div>
    </div>
  );
}
...
export default function AboutPage() {
  useRevealAll();
  const { members, stats: allStats } = useAdmin();
    ...
    <div className="bg-white">
      ...
      >
        ...
        <div className="relative z-10 px-6 pt-16">
          ...
          <p className="mt-4 text-xs sm:text-sm font-bold uppercase tracking-[0.3em] text-white/80">
            About Us
          </p>
        </div>
      </section>
      ...
      <section className="pt-16 pb-10 lg:pt-20 lg:pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="reveal-left">
            <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#3BE3A0]">
              Who We Are
            </span>
            <h2 className="text-3xl lg:text-4xl font-extrabold text-[#072069] mt-3 leading-tight">
            ...
            <p className="text-[#676F7E] mt-4 leading-relaxed">
              ...
              infrastructure. We combine technical excellence with deep industry
              knowledge to deliver solutions that create real value.
            </p>
            <p className="text-[#676F7E] mt-4 leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-8">
              {stats.map((stat) => (
                <div key={stat.id} className="text-center">
                  <span className="mx-auto mb-2 flex h-9 w-9 items-center justify-center rounded-full bg-white text-[#072069]">
                    <StatIcon icon={stat.icon} className="h-4.5 w-4.5" />
                  </span>
                  <p className="text-3xl sm:text-4xl font-extrabold text-[#072069]">
                    {stat.value}
                  </p>
      ...
      <section className="about-identity pb-16 lg:pb-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-3xl lg:text-4xl font-extrabold text-[#072069] reveal">
            Our Identity,
            <br />
            Vision and Values
          </h2>
            ...
            {interns.length > 0 && (
              <div className="text-center mt-12 reveal">
                <button
                  ...
                  className="btn-navy px-7 py-3.5 rounded-xl text-sm font-semibold inline-flex items-center gap-2"
                >
        ...
        <section id="intern-team" className="pb-20 lg:pb-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center reveal">
              <h2 className="text-3xl lg:text-4xl font-extrabold text-[#072069]">
                Our Intern Team
              </h2>
              <p className="text-[#676F7E] mt-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 max-w-3xl mx-auto mt-10">
              {interns.map((member) => (
                <InternCard key={member.id} member={member} />
              ))}
            </div>
          </div>
        </section>
